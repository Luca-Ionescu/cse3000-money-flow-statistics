...
def build_g(casename):
    ...
    datatu=pd.read_csv(f'./DenseFlow_Tool/inputData/AML/'+casename+'/all-normal-tx.csv')
    ...
    for i in range(row):
        ...
        #筛选部分交易
        if True or datatu.loc[i,'isError']== 0:
...
def output_flow_add(casename,k,level,sourceadd,Show = False, Save = True):
    ...
    print('读取对应heist地址...')
    heist1 = pd.read_excel(f'./DenseFlow_Tool/inputData/AML/'+casename+'/myheist_k_'+str(k)+'.xlsx',sheet_name='Sheet'+str(level),index_col=0)
    ...
    if Save:
        print('地址存储中...')
        out_path =f'./DenseFlow_Tool/inputData/AML/{casename}/'+casename+"_out/"
