...
def csvtodata(casename):
    ...
    #不要去重，统计边数
    relation_count = []
    relation_tuples = [tuple(edge) for edge in relation]
    relation_count = pd.Series(relation_tuples).value_counts()

    #relation_count = pd.Series(relation).value_counts()
    ...
    mkdir('data/'+casename+"/")
    #输出from文件
    dict_from_path = './DenseFlow_Tool/inputData/AML/' + casename + '/all-normal-tx_from.txt'
    ...
    # 输出to文件
    dict_to_path = './DenseFlow_Tool/inputData/AML/' + casename + '/all-normal-tx_to.txt'
...
#     print(relationship_df)
    relationship_df.to_csv('./DenseFlow_Tool/inputData/AML/'+casename+'/all-normal-tx_fraudar.csv',header=0,index=False)
    txtname = './DenseFlow_Tool/inputData/AML/' + casename + '/all-normal-tx'+ 'count.txt'
...
def datasetoverview(casename):
    ...
    heist = pd.read_csv('./DenseFlow_Tool/inputData/AML/'+casename+'/accounts-hacker.csv')
